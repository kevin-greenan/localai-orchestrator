...
def _effective_image_gen_url(cfg: StackConfig) -> str:
    runtime_env = _read_runtime_env(Path(cfg.root))
    port = runtime_env.get("LOCALAI_IMAGE_GEN_PORT")
    if port and port.isdigit():
        return f"http://127.0.0.1:{port}/healthz"
    return "http://127.0.0.1:8090/healthz"


def _effective_minio_url(cfg: StackConfig) -> str:
    runtime_env = _read_runtime_env(Path(cfg.root))
    port = runtime_env.get("LOCALAI_MINIO_PORT")
    if port and port.isdigit():
        return f"http://127.0.0.1:{port}/minio/health/live"
    return "http://127.0.0.1:9000/minio/health/live"
...
def _write_runtime_env(
    ...
    *,
    ...
    minio_user = runtime_env.get("LOCALAI_MINIO_ROOT_USER") or "localai"
    minio_password = runtime_env.get("LOCALAI_MINIO_ROOT_PASSWORD") or secrets.token_hex(16)
    image_gen_port = runtime_env.get("LOCALAI_IMAGE_GEN_PORT") or "8090"
    minio_port = runtime_env.get("LOCALAI_MINIO_PORT") or "9000"
    minio_console_port = runtime_env.get("LOCALAI_MINIO_CONSOLE_PORT") or "9001"
    image_gen_public = runtime_env.get("LOCALAI_IMAGE_GEN_PUBLIC_BASE_URL") or f"http://127.0.0.1:{image_gen_port}"
    lines = [
        ...
        f"LOCALAI_MINIO_CONSOLE_PORT={minio_console_port}",
        "LOCALAI_MINIO_ENDPOINT=http://minio:9000",
        "LOCALAI_MINIO_BUCKET=generated",
...
def _cmd_up(args: argparse.Namespace) -> int:
    ...
    if cfg.image_gen.enabled:
        for service in ["image-gen", "minio", "image-redis"]:
            if service not in cfg.docker.services:
                cfg.docker.services.append(service)
    else:
        cfg.docker.services = [s for s in cfg.docker.services if s not in {"image-gen", "minio", "image-redis"}]
...
def _cmd_doctor(args: argparse.Namespace) -> int:
    ...
    elif cfg.image_gen.enabled:
        checks.append(("image-gen http", True, "skipped (service not enabled)"))
    if "minio" in active_services:
        minio_ok, minio_msg = http_ok(_effective_minio_url(cfg))
        checks.append(("minio http", minio_ok, minio_msg[:120]))
    elif cfg.image_gen.enabled:
        checks.append(("minio http", True, "skipped (service not enabled)"))
